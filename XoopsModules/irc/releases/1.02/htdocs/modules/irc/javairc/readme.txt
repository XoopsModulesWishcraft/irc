...
-----
  ...
  The following ".$GLOBALS['xoopsDB']->prefix("files")." are part of the IRC engine :
    ...
    The cab ".$GLOBALS['xoopsDB']->prefix("files")." are only recognized by Internet Explorer. jar files
...
Installation
------------
	Just upload the ".$GLOBALS['xoopsDB']->prefix("files")." you need, in regard to the mode (signed or unsigned)
	you want. Upload any extra files, such as language ".$GLOBALS['xoopsDB']->prefix("files")." or images and
	sound.
...
-----------------
  ...
  bound to a DCC server. Here is the list of all known sources :
    ...
    - DCCFile : bound to a DCC server, the DCCFile is a special
                source used for transmitting ".$GLOBALS['xoopsDB']->prefix("files")." from a client
...
Have a look at http://www.pjirc.com/ for ".$GLOBALS['xoopsDB']->prefix("news")." about PJIRC.
